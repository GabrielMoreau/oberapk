...
function oberpakaj_toggle_screens {
   ...
   then
      ...
      git clone https://gricad-gitlab.univ-grenoble-alpes.fr/legi/soft/trokata/toggle-screens.git
   fi
   ...
   then
      cd "${HOME}/upload/${pakajname}" || return
      ls -1t -- toggle-screens_*.deb 2> /dev/null | tail -n +$((keep+1)) | xargs -r rm -f --
   fi
   }
