...
function oberpakaj_yadedaily {
   ...
   ls -1t -- libyadedaily_*.deb      2> /dev/null | tail -n +$((keep+1)) | xargs -r rm -f --
   ls -1t -- python3-yadedaily_*.deb 2> /dev/null | tail -n +$((keep+1)) | xargs -r rm -f --
   ls -1t -- yadedaily_*.deb         2> /dev/null | tail -n +$((keep+1)) | xargs -r rm -f --
   done
   }
